...
const SUPABASE_ANON_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1zemJobG1panVmc2Nob3NzZmNqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk4ODQ2MjksImV4cCI6MjA5NTQ2MDYyOX0.J1pWMpHVbRJzjIV6HrbR-0LcCe09J8yuE_WMuuYOkqo';
...
function render() {
  ...
  grid.innerHTML = items
    .map((p, i) => {
      ...
      return `
      <div class="product-card" data-id="${p.id}" style="animation-delay:${i * 40}ms">
        <div class="img-wrap">
          <img src="${p.image_url}" alt="${escapeHtml(p.name)}" loading="lazy" />
          ${hasStrike ? '<span class="sale-badge">Sale</span>' : ''}
...
function openProduct(id) {
  ...
  const hasStrike = p.strike_rate && p.strike_rate > p.rate;
  document.getElementById('m-strike').textContent = hasStrike
    ? `Rs. ${p.strike_rate.toLocaleString('en-IN')}.00`
    : '';
  ...
  if (p.sizes && p.sizes.length) {
    ...
    sizesEl.innerHTML = p.sizes
      .map((s) => `<button class="opt" data-size="${escapeHtml(s)}">${escapeHtml(s)}</button>`)
      .join('');
  ...
  if (p.colors && p.colors.length) {
    ...
    colorsEl.innerHTML = p.colors
      .map((c) => `<button class="opt" data-color="${escapeHtml(c)}">${escapeHtml(c)}</button>`)
      .join('');
